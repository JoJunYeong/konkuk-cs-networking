...
참가 신청과 결제는 ZADU 시스템과 분리해 외부 행사 플랫폼에서 처리합니다. 공개 페이지는 일정과 모임 취지를 설명하고, 회차별 신청 URL이 연결되면 해당 플랫폼으로 이동합니다. URL이 없을 때는 개인정보를 받지 않고 `신청 링크 준비 중` 상태만 표시합니다.
...
- 회차별 온오프믹스 신청·결제 URL 연결
- 신청자·결제·공지 관리 화면 바로가기
