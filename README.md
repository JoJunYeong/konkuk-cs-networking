...
건국대학교 컴퓨터공학 편입생·재학생·졸업생 네트워킹 데이를 분기마다 운영하기 위한 행사 페이지입니다. 로우로우·자두의 밝고 간결한 화면을 참고한 KU meet 디자인으로, 첫 화면에서 일정과 참가비를 확인하고 두 단계로 신청할 수 있습니다. 2023년 실제 운영 기록은 개인정보 없이 아카이브에 담았습니다.

참가 신청은 회차별로 외부 행사 플랫폼 또는 자체 계좌이체 방식을 고를 수 있습니다. 자체 방식에서는 이름·휴대전화·입금자명만 비공개 Firestore 문서에 접수하고, 운영자가 은행 입금 내역을 일괄 대조한 뒤 참여 확정으로 바꿉니다. 신청자가 누른 `입금완료 · 참여신청 마무리`는 신청 접수와 입금확인 요청입니다. 실제 입금은 운영자가 확인합니다.
...
관리자 화면에서 회차의 신청 방식을 `계좌이체`로 고르고 은행·예금주·계좌번호·참가비·환불 마감 안내를 입력합니다. `접수 중` 상태와 계좌 정보가 모두 갖춰져야 공개 신청과 Firestore 모집 게이트가 함께 열립니다. 접수 마감 일시는 한국 시간으로 설정하며, 그 시각부터 신규 신청을 차단합니다.

신청자는 이름·휴대전화·실제 입금자명과 개인정보 동의를 입력한 뒤 입금 안내로 이동합니다. `계좌번호만 복사`는 은행명·예금주·공백·하이픈을 제외한 숫자만 복사합니다. 송금 후 `입금완료 · 참여신청 마무리`를 누르면 신청이 저장되고 완료 화면과 신청번호가 표시됩니다. 뒤로 가도 입력값은 유지되며, 같은 브라우저에서는 새로고침 후에도 신청번호를 볼 수 있습니다. 이 영수증은 접수 기록이며 운영자의 최신 입금 확정 상태를 조회하는 화면은 아닙니다. 운영자는 `신청·입금` 탭에 은행 내역의 `입금자명[TAB]금액` 두 열을 붙여 넣어 유일하게 일치한 건을 한 번에 확정합니다. 동명이인과 중복 건은 자동 선택하지 않습니다.
